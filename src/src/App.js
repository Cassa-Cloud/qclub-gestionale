...
const KEYS = { res: "qclub-res", bot: "qclub-bot", prs: "qclub-prs", ts: "qclub-ts" };
...
function printCashierList(reservations, bottles) {
  ...
  .footer{display:flex;justify-content:space-between;margin-top:14px;font-size:11px;color:#94a3b8}
  ...
  <div class="footer"><span>Stampato il ${new Date().toLocaleString("it-IT")}</span><span>Q Club Gestionale</span></div>
  </body></html>`;
...
function printFloorPlan(reservations) {
  ...
  const tableHTML = TABLES.map(t => {
    const r = reservations[t.id]; const zm = zmPrint[t.zone];
    return `<div style="position:absolute;left:${t.x}%;top:${t.y}%;width:${t.w}%;height:${t.h}%;
      background:${r?zm.bg:"#f8fafc"};border:2px solid ${r?zm.border:"#e2e8f0"};border-radius:6px;
      ...
      ${r
        ? `<div style="font-size:7px;color:${zm.color};max-width:94%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center">${r.clientName}</div>`
        : `<div style="font-size:7px;color:#cbd5e1">libero</div>`}
    </div>`;
  }).join("");
  const legendItems = Object.entries({Console:"#fef3c7",Perimetrale:"#dbeafe",Centrale:"#fee2e2",Mensola:"#f3e8ff",Lounge:"#d1fae5"})
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
    ...
    <div style="display:flex;gap:10px;font-size:10px;align-items:center">${legendItems}
...
function AppInner({ role, roleLabel, isCassa, onLogout }) {
  ...
  const sync = useCallback(()=>{
    ...
    if (ts !== lastTs) {
      setReservations(load(KEYS.res,{})); setBottles(load(KEYS.bot,DEFAULT_BOTTLES)); setPrs(load(KEYS.prs,DEFAULT_PRS)); setLastTs(ts);
    }
    ...
    <div style={{ minHeight:"100vh",background:"#f0f2f5",fontFamily:"Georgia,serif",color:"#1a1a2e" }}>
      ...
      <div style={{ background:"#fff",borderBottom:"2px solid #e5e7eb",overflowX:"auto" }}>
        <div style={{ maxWidth:1100,margin:"0 auto",display:"flex" }}>
          {[{key:"map",label:"🗺 Mappa"},{key:"list",label:"📋 Ospiti"},{key:"bottles",label:"🍾 Bottiglie"},{key:"prs",label:"👤 PR"},{key:"report",label:"📊 Report"}].map(t=>(
          ...
          <div>
            ...
            <div style={{ display:"flex",gap:10,marginBottom:16,justifyContent:"space-between",flexWrap:"wrap",alignItems:"center" }}>
              ...
              <div style={{ display:"flex",gap:8 }}>
                <button onClick={()=>printCashierList(reservations,bottles)} style={{ padding:"8px 16px",background:"#fff",border:"1px solid #e2e8f0",borderRadius:8,fontSize:12,cursor:"pointer",fontWeight:600,color:"#1a1a2e" }}>🖨 Lista Cassiere</button>
                <button onClick={()=>printFloorPlan(reservations)} style={{ padding:"8px 16px",background:"#fff",border:"1px solid #e2e8f0",borderRadius:8,fontSize:12,cursor:"pointer",fontWeight:600,color:"#1a1a2e" }}>🗺 Stampa Piantina</button>
              </div>
            </div>
        ...
        {tab==="prs"&&(
          <div>
            ...
            <div style={{ background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:8,padding:"10px 16px",marginBottom:20,fontSize:12,color:"#92400e",lineHeight:1.7 }}>
              <strong>Scaglioni provvigione:</strong> tutti partono al <strong>10%</strong> · se fattura &gt;€250 → <strong>15%</strong> (solo se attivato sul PR) · se fattura &gt;€1000 → <strong>20%</strong> (solo se attivato sul PR)
            </div>
            {isCassa&&<div style={{ background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:20,marginBottom:20 }}>
              ...
              <div style={{ display:"flex",gap:12,flexWrap:"wrap",alignItems:"flex-end" }}>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                  ...
                  <label style={{ display:"flex",alignItems:"center",gap:8,fontSize:13,cursor:"pointer" }}>
                    <input type="checkbox" checked={prForm.tier250} onChange={e=>setPrForm(p=>({...p,tier250:e.target.checked}))} style={{ width:16,height:16 }} />
                    <span>15% sopra €250</span>
                  </label>
                  <label style={{ display:"flex",alignItems:"center",gap:8,fontSize:13,cursor:"pointer" }}>
                    <input type="checkbox" checked={prForm.tier1000} onChange={e=>setPrForm(p=>({...p,tier1000:e.target.checked}))} style={{ width:16,height:16 }} />
                    <span>20% sopra €1000</span>
                  </label>
                </div>
                <button onClick={()=>{
                    ...
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12 }}>
                      ...
                      {isCassa&&<div style={{ display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end" }}>
                        ...
                        <label style={{ display:"flex",alignItems:"center",gap:4,fontSize:11,cursor:"pointer",color:"#64748b" }}>
                          <input type="checkbox" checked={!!pr.tier250} onChange={e=>setPR(prs.map(p=>p.id===pr.id?{...p,tier250:e.target.checked}:p))} />15%
                        </label>
                        <label style={{ display:"flex",alignItems:"center",gap:4,fontSize:11,cursor:"pointer",color:"#64748b" }}>
                          <input type="checkbox" checked={!!pr.tier1000} onChange={e=>setPR(prs.map(p=>p.id===pr.id?{...p,tier1000:e.target.checked}:p))} />20%
                        </label>
                      </div>}
                    </div>
          ...
          <div>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10 }}>
              ...
              <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                ...
                {isCassa&&<button onClick={()=>{if(window.confirm("Azzerare tutta la serata?")){setRes({});toast$("Serata azzerata ✓");}}} style={{ padding:"8px 14px",background:"#fee2e2",color:"#dc2626",border:"1px solid #fca5a5",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:600 }}>🔄 Nuova Serata</button>}
                <button onClick={()=>window.print()} style={{ padding:"8px 18px",background:"#1a1a2e",color:"#fff",border:"none",borderRadius:7,cursor:"pointer",fontSize:12 }}>🖨 Report Completo</button>
              </div>
            </div>
                ...
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
                    <thead><tr style={{ borderBottom:"2px solid #f1f5f9" }}>
                      {["Bottiglia","Cat.","Qt.","Rimaste","Costo Tot.","Ricavo","Margine"].map(h=><th key={h} style={{ padding:"8px 10px",textAlign:"left",fontSize:11,color:"#94a3b8" }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                      {bottleUsage.map(b=>(
            ...
            <div style={{ background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:20 }}>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
                  <thead><tr style={{ borderBottom:"2px solid #f1f5f9" }}>
                    {["PR","Scaglioni","Tavoli","Incasso Totale","% Applicata","Provvigione"].map(h=><th key={h} style={{ padding:"8px 10px",textAlign:"left",fontSize:11,color:"#94a3b8" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {prReport.map(pr=>(
                      <tr key={pr.id} style={{ borderBottom:"1px solid #f8fafc" }}>
                        ...
                        <td style={{ padding:"10px" }}>
                          <div style={{ display:"flex",gap:4,flexWrap:"wrap" }}>
                            <span style={{ fontSize:10,background:"#f1f5f9",borderRadius:3,padding:"1px 6px",color:"#64748b" }}>10%</span>
                            {pr.tier250&&<span style={{ fontSize:10,background:"#fef3c7",borderRadius:3,padding:"1px 6px",color:"#b45309" }}>15%</span>}
                            {pr.tier1000&&<span style={{ fontSize:10,background:"#dcfce7",borderRadius:3,padding:"1px 6px",color:"#16a34a" }}>20%</span>}
                          </div>
                        </td>
                    ...
                    <tr style={{ background:"#f8fafc",fontWeight:700 }}>
                      <td colSpan={5} style={{ padding:"10px",color:"#64748b",fontSize:12 }}>TOTALE PROVVIGIONI</td>
